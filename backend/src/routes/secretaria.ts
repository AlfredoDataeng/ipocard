...
export default async function secretariaRoutes(fastify: FastifyInstance) {
  ...
  // --- MENU CANTINA (PRODUCTS CRUD) ---

  // Create Product
  fastify.post('/products', async (request, reply) => {
    const { name, price, category, imageUrl } = request.body as any;
    if (!name || price === undefined || !category) {
      return reply.code(400).send({ error: 'Nome, preço e categoria são obrigatórios.' });
    }
  ...
  fastify.delete('/products/:id', async (request, reply) => {
    const { id } = request.params as any;
    try {
      // Historical purchases use JSON snapshot, so deleting is perfectly safe.
