...
exports.default = secretariaRoutes;
const bcryptjs_1 = __importDefault(require("bcryptjs"));
async function secretariaRoutes(fastify) {
    ...
    fastify.post('/students', async (request, reply) => {
        const { name, classGroup, photoUrl, password } = request.body;
        if (!name || !classGroup) {
    ...
    fastify.post('/students/:id/deposit', async (request, reply) => {
        const { id } = request.params;
        const { amount, receiptRef } = request.body;
    ...
    fastify.put('/students/:id', async (request, reply) => {
        const { id } = request.params;
        const { name, classGroup } = request.body;
        if (!name || !classGroup) {
            return reply.code(400).send({ error: 'Nome e turma são obrigatórios.' });
        }
        try {
